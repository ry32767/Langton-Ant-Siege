...
// DOM に依存しない純粋な ESM。エンジンの挙動は必ず src/engine.js の simulateVersus を経由する
// (自前でシミュレーションを書き直さない。AGENTS.md「エンジンの参照実装」)。
import { simulateVersus } from '../engine.js';
...
/**
 * genome 形({ cells: [{x,y,state}] })と template 形({ cells: [[x,y,state]] })の
 * どちらが渡されても engine.js が期待する template 形に揃える。
 */
function toEngineTemplate(source, kind) {
  if (!source) return source;
  const rawCells = source.cells ?? [];
  const cells = rawCells.map((c) => (Array.isArray(c) ? c : [c.x, c.y, c.state]));
  return {
    rule: source.rule,
    cells,
    antX: source.antX,
 ...
 *   - attackFinalX / disruptFinalX / disruptFinalY (simulateVersus はどのアリの最終座標も返さない)
 *   - attackFinalY (didScore=true の時の entryY だけは取れるが、消滅で終わった場合は null のまま)
 ...
 *     trackPath に対応していない)
 */
export function evaluateInteraction(attack, disrupt, fireAtStep, options = {}) {
  const attackTpl = toEngineTemplate(attack, 'attack');
  const opponents = [];
  if (disrupt) {
    const disruptTpl = toEngineTemplate(disrupt, 'disrupt');
 ...
 */
function simulateWithEscort(attack, intercept, interceptFireAtStep, escort, escortFireAtStep, options) {
  const attackTpl = toEngineTemplate(attack, 'attack');
  const interceptTpl = toEngineTemplate(intercept, 'disrupt');
  const escortTpl = toEngineTemplate(escort, 'disrupt');
  const opponents = [
...
/**
 * attacks × disrupts × C.FIRE_TIMINGS の全組み合わせを評価する。
 * 「攻撃が得点しなかった時点で残りタイミングを打ち切る」ような枝刈りはしない
 * (各タイミングは独立に評価する。タイミングそのものが効くかどうかを見る評価器のため)。
 *
 * options.onProgress({ evaluated, total, attackId, disruptId, fireAtStep }) を
...
export function buildCounterMatrix(attacks, disrupts, options = {}) {
  ...
  const total = attacks.length * disrupts.length * timings.length;
  let evaluated = 0;

  for (let i = 0; i < attacks.length; i++) {
    const attack = attacks[i];
    const attackId = idOf(attack, i);
    matrix[i] = [];
    let stoppedCountForAttack = 0;
    const entries = [];

    for (let j = 0; j < disrupts.length; j++) {
      const disrupt = disrupts[j];
      const disruptId = idOf(disrupt, j);
      if (!disruptStopped.has(disruptId)) disruptStopped.set(disruptId, false);
      matrix[i][j] = [];

      let stoppedCountForPair = 0;
      const stoppedTimings = [];
      for (let t = 0; t < timings.length; t++) {
        const fireAtStep = timings[t];
        const { scored } = evaluateInteraction(attack, disrupt, fireAtStep, options);
        matrix[i][j][t] = scored;
        evaluated++;
        if (typeof options.onProgress === 'function') {
          options.onProgress({ evaluated, total, attackId, disruptId, fireAtStep });
        }
        if (!scored) {
          stoppedCountForPair++;
      ...
      const successRate = stoppedCountForPair / timings.length;
      for (const fireAtStep of stoppedTimings) {
        entries.push({ disruptId, fireAtStep, successRate });
      }
    }

    counterTable[attackId] = entries;
    counterDensity[i] = disrupts.length > 0 ? stoppedCountForAttack / (disrupts.length * timings.length) : 0;
...
/**
 * counterTable に記録された「攻撃を止めた迎撃(disruptId, fireAtStep)」ごとに、
...
export function buildEscortTable(attacks, disrupts, counterTable, options = {}) {
  ...
  for (let i = 0; i < attacks.length; i++) {
    ...
    // 迎撃 disruptId ごとに、それが攻撃を止めた fireAtStep 一覧をまとめる。
    const byIntercept = new Map();
    for (const e of entries) {
      if (!byIntercept.has(e.disruptId)) byIntercept.set(e.disruptId, []);
      byIntercept.get(e.disruptId).push(e.fireAtStep);
    }

    const perAttack = {};
    for (const [interceptId, interceptTimings] of byIntercept.entries()) {
      ...
      const intercept = disrupts[interceptIndex];

      const found = [];
      for (const interceptFireAtStep of interceptTimings) {
        let escortFound = null;
        for (let j = 0; j < disrupts.length && !escortFound; j++) {
          const escort = disrupts[j];
          const escortId = idOf(escort, j);
          for (const escortFireAtStep of timings) {
            ...
            if (result.scored) {
              escortFound = { escortDisruptId: escortId, fireAtStep: escortFireAtStep };
              break;
            }
          }
        }
        if (escortFound) found.push({ interceptFireAtStep, ...escortFound });
      }
