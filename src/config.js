 ...
 * 必要な時間が足りない(CPU の判断周期は DECISION_INTERVAL_STEPS = 1秒なので、
 * CPU 相手でも判断4回ぶんしかない)。670 は最速弾5.2秒を確保できる下限として選んだ。
 *
 * ⚠️ この値を変えたら FIRE_TIMING_INTERVAL が DECISION_INTERVAL_STEPS(=この値)の
 * 倍数になっていないかを必ず確認する(test/config.test.js が検査する)。倍数になると
