...
妨害の正確な照準は学習対象にしない。**v5.5で3×3に削減**(攻撃3種×妨害3種)。有限手札について全ペア総当たりで得た情報を実行時に捨てて再学習することを禁止する。
...
10. ブラウザ実行時、`src/app.js` が両JSONを読み込み、プレイヤーには攻撃/妨害のタブで3種ずつ役割別に一覧提示(v5.5)、`src/cpu.js` は方策に従って選択(実行時の再シミュレーションは行わない)。発射列(`antX`)の選択とテンプレートの絶対座標化は `src/template.js` が担う
