...
| **防御手段** | **安価な妨害アリ(コスト3・寿命6,000。v5.3。v5.2までは1,000)** | v3.1実測で、配置ブロックによる防御はまったく効かない(80個置いても得点率が下がらない)。一方、狙って撃った妨害アリは98%の確率で攻撃を止める | 配置ブロックで防ぐ(実測で機能しない) / 得点条件を「帯の突破」にする(機能するが、駆け引きが静的) |
...
| CPUの構成 | **戦術=事前計算テーブル / 戦略=自己対戦で学習** | 9種しかないので「どの札がどの札に勝つか」は全ペア総当たりで事前計算できる(カウンター表・護衛表)。残る判断は「いつ何にトークンを使うか・いつ自爆するか・攻撃の発射列をどう選ぶか」だけで、パラメータ17個(v5.4。v5.3 までは18個、v5.1 までは13個)の小さな方策になる | 全部を学習させる(状態空間が無駄に大きい) / 全部を手書きヒューリスティックにする(調整が終わらない) |
| CPUの学習手法 | CEM法(交差エントロピー法)による自己対戦 | パラメータ17個(v5.4)なので勾配不要。v5.2実測で **約115試合/秒**、集団60×40試合×40世代 = 96,200試合で **約14分**。1世代の中は共通乱数(CRN)で揃え、fitness は「現在の平均方策との勝率」と「無学習の中立方策(固定アンカー)との勝率」の平均にする。詳細は spec.md 決定事項参照 | 強化学習(この規模では過剰。Phase 2) |
...
妨害の正確な照準は学習対象にしない。9×9の有限手札について全ペア総当たりで得た情報を実行時に捨てて再学習することを禁止する。
